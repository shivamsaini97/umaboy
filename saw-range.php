<?php include('header.php'); ?>

<div class="home-slider-area">
  <div id="welcome-slide-carousel" class="carousel slide carousel-fade" data-ride="carousel">
   
    <div class="carousel-inner" role="listbox"> 
    
      <div class="item active">
        <div class="single-slide-item slide-5">
          <div class="single-slide-item-table">
            <div class="single-slide-item-table-cell">
              <div class="container">
                <div class="row">
                  <div class="col-md-12">
                    <h2 style="text-transform:capitalize">SAW Range</span></h2>
                   
                   <!-- <a class="slide-btn" href="javascript:void(0)">Contact Us</a>--></div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      
      
 </div>
  </div>
</div>





<div class="blog-wrapper">
  <div class="container">
    <div class="title">
      <h2>Our Products</h2><hr/>
      </div>
    <div class="row">
      <div class="col-lg-4 col-md-6 col-sm-6">
        <div class="single-blog-section"> 
          <a href=""><img src="images/saw-range1.png" alt=""></a>
          <div class="blog-info">
            
            <div class="content">
              <a href=""><h3>Push Button</h3></a>
              <div class="collor-bottom-two"></div>
            </div>
          
            <button class="btn btn-default unicod-btn-black" type="submit"><a href="product-details.html">read more</a></button>
          </div>
        </div>
      </div>
    
   <div class="col-lg-4 col-md-6 col-sm-6">
        <div class="single-blog-section"> 
          <a href="beam-saw-detail.html"><img src="images/saw-range2.png" alt=""></a>
          <div class="blog-info">
            
            <div class="content">
              <a href="beam-saw-detail.html"><h3>BEAM SAW</h3></a>
              <p>With Optimizer Software</p>
              <div class="collor-bottom-two"></div>
            </div>
          
            <button class="btn btn-default unicod-btn-black" type="submit"><a href="beam-saw-detail.html">read more</a></button>
          </div>
        </div>
      </div>
      <div class="col-lg-4 col-md-6 col-sm-6">
        <div class="single-blog-section"> 
          <a href="">
            <img src="images/saw-range3.png" alt="">
          </a>
          <div class="blog-info">
            
            <div class="content">
              <a href=""><h3>Panel Saw</h3></a>
              <p>5 Hp, 7 Hp, Manual, Semi-Automatic, Fully Automatic</p>
              <div class="collor-bottom-two"></div>
            </div>
          
            <button class="btn btn-default unicod-btn-black" type="submit"><a href="product-details.html">read more</a></button>
          </div>
        </div>
      </div>
      
      
    </div>
  </div>
</div>

<?php include('footer.php'); ?>