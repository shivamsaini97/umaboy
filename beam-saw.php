<?php include('header.php'); ?>

<div class="home-slider-area">
  <div id="welcome-slide-carousel" class="carousel slide carousel-fade" data-ride="carousel">
   
    <div class="carousel-inner" role="listbox"> 
    
      <div class="item active">
        <div class="single-slide-item slide-5">
          <div class="single-slide-item-table">
            <div class="single-slide-item-table-cell">
              <div class="container">
                <div class="row">
                  <div class="col-md-12">
                    <h2 style="text-transform:capitalize">Beam Saw</span></h2>
                   <!-- <a class="slide-btn" href="javascript:void(0)">Contact Us</a>-->
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      
      
 </div>
  </div>
</div>





<div class="blog-wrapper">
  <div class="container">
    <div class="title">
      <h2>Our Products</h2><hr/>
      </div>
    <div class="row">
      <div class="col-lg-4 col-md-6 col-sm-6">
        <div class="single-blog-section"> 
          <a href=""><img src="images/beam-saw-auo.png" alt=""></a>
          <div class="blog-info">
            
            <div class="content">
              <a href=""><h3>Beam Saw UBS 3200 Auto</h3></a>
              <p>Maximum production output with excellent cutting of multiple panels</p>
              <div class="collor-bottom-two"></div>
            </div>
          
            <button class="btn btn-default unicod-btn-black" type="submit"><a href="beam-saw-auto-detail.html">read more</a></button>
          </div>
        </div>
      </div>
    
   <div class="col-lg-4 col-md-6 col-sm-6">
        <div class="single-blog-section"> 
          <a href="beam-saw-detail.html"><img src="images/beamsaw.png" alt=""></a>
          <div class="blog-info">
            
            <div class="content">
              <a href="beam-saw-detail.html"><h3>Beam Saw UBS 3200</h3></a>
              <p>Maximum production output with excellent cutting of multiple panels</p>
              <div class="collor-bottom-two"></div>
            </div>
          
            <button class="btn btn-default unicod-btn-black" type="submit"><a href="beam-saw-detail.html">read more</a></button>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<?php include('footer.php'); ?>